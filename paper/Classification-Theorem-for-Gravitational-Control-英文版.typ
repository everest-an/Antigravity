...
with $phi$ the KK scalar mode and $Lambda$ the EFT cutoff. This is the leading state-distinguishing operator: $Q_("ij") phi$ carries open indices and is not a Lorentz scalar, and the dimension-5 spin coupling requires an external magnetic field and vanishes with an unpolarized source—so dimension 6 is the first state-distinguishing term in the static-source geometry. In the static limit $partial^2 phi arrow.r m_"KK"^2 phi = phi/lambda^2$. The ratio of the state-dependent potential to the universal Yukawa potential is

$ (delta alpha)/(alpha) = (c Q_"nuc" M_"Pl")/(sqrt(alpha) Lambda^2 lambda^2 m_"nuc"). $ <eq-sdratio>

Four remarks. (i) $delta alpha$ grows as $sqrt(alpha)$—the state-dependent channel is relatively stronger for smaller $alpha$, but its absolute size remains far below reachability throughout the window (see the tables of script 17). (ii) Observability requires the cutoff to drop to about 43 MeV (at the window edge $alpha = 10^(-6)$; about 1 MeV at the reachability edge $alpha = 10^(-12)$)—that is, below the hadronic/QCD scale, where the local EFT description itself breaks down. (iii) New physics with a MeV cutoff at macroscopic $lambda$ was long excluded by fifth-force tests. (iv) $c ~ O(1)$ is the standard EFT naturalness convention (in the absence of a protecting symmetry, Wilson coefficients have no reason to deviate significantly from one); even for $c$ as small as $10^(-3)$ the zero-background conclusion survives (the gap merely shrinks by 3 orders of magnitude). Together: *the state-dependent signal is observable if and only if the local EFT description fails*—Corollary 4's zero-background criterion is thereby established; all numbers come from experiments/17, and Fig. 7 provides the visualization.
