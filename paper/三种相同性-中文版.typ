...
*摘要*　"统一场论"一词被用于三类逻辑上不同的断言：转换式（一种场物理地转化为另一种场）、几何式（多种场是高维几何的投影）、代数式（多种理论共享更深的代数结构）。本文证明：这三类穷尽了所有"物理现象 A 与 B 是同一件事"的可检验断言——相同性断言只有三种形式：映射、共同投影、共享不变量。证明以理论的形式化框架（理论即范畴，Halvorson 2019）与对偶等价的形式化文献（De Haro-Butterfield 2018；Weatherall 2018）为语言。三类形式各自对应一种证据格局（定量判决/窗口收窄/本体沉默），从而解释了为什么三类统一以不同方式失败、需要不同实验判决——这正是判决矩阵方法论的形式基础。本文包含显式证伪条款：若发现第四种相同性结构，分类即被修正。
...
5. Bern, Z., Carrasco, J. J. & Johansson, H. (double copy 纲领综述系列).
6. Kaluza, T. (1921); Klein, O. (1926)（纤维化原型）.
7. MuningAn. 三种统一：量子引力前沿的一张可证伪地图（2026）.
