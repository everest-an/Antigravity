...
*摘要*　本文在广义相对论加标准模型的有效场论（EFT）框架内证明引力控制通道的分类定理：所有作用于测试粒子运动的低能、洛伦兹不变的度规-物质耦合按对称性与维数分为三类——源通道（Tμν 及其高阶维算符的极小耦合，即能量/压强/流工程）、几何通道（曲率耦合项，被 1/M_Pl 或观测约束压制）、新扇区通道（超出标准模型的新场，受第五力与洛伦兹不变性检验约束）。定理的推论是：引力工程在 GR+SM 内等价于应力能量工程；任何"反重力"装置必须落入三类通道之一，而每一类都已被定量约束。证明由四根支柱支撑：正能定理（主能条件下孤立系统无净排斥引力）、EFT 维数分析（曲率项不可达）、PPN 与引力波观测（几何通道系数的压制）、量子不等式（源通道的量子边界）。定理与曲速引擎 no-go 文献的关系是包含关系，文中给出差异声明。
...
*证伪条款*：本定理在四种情形下失效，且失效即发现：(1) 主能条件被实验违反（源通道重开）；(2) 某曲率耦合的观测系数远大于 $1/M_"Pl"^2$ 压制（几何通道重开）；(3) 第五力/LIV 检验发现显著信号（新扇区通道打开）；(4) 量子叠加源或涌现几何机制（定理不覆盖，对应判决矩阵第 6-7、18、20 行）。四种情形各有已排期的判决实验——定理的可证伪性由此被显式化。

*边界*：定理是分类陈述而非发现陈述；它不预言新现象，它精确说明旧直觉为何失败及失败备选出口的位置。

= 结论

在 GR+SM 的 EFT 框架内，引力控制通道被分类为源、几何、新扇区三类，三类各有定量边界，边界各有可执行脚本。定理把"反重力是否可能"从传说问题转化为带参数区的工程问题；其证伪条款明确列出四条重开条件，每条对应一个正在排期的实验。
