...
*摘要*　本文在广义相对论加标准模型的有效场论（EFT）框架内证明引力控制通道的分类定理：所有作用于测试粒子运动的低能、洛伦兹不变的度规-物质耦合按对称性与维数分为三类——源通道（Tμν 及其高阶维算符的极小耦合，即能量/压强/流工程）、几何通道（曲率耦合项，被 1/M_Pl 或观测约束压制）、新扇区通道（超出标准模型的新场，受第五力与洛伦兹不变性检验约束）。定理的推论是：引力工程在 GR+SM 内等价于应力能量工程；任何"反重力"装置必须落入三类通道之一，而每一类都已被定量约束。证明由四根支柱支撑：正能定理（主能条件下孤立系统无净排斥引力）、EFT 维数分析（曲率项不可达）、PPN 与引力波观测（几何通道系数的压制）、量子不等式（源通道的量子边界）。定理与曲速引擎 no-go 文献的关系是包含关系，文中给出差异声明。推论 4 给出态相关耦合的 EFT 上界：核内部态对 Yukawa 耦合的区分被压制约 16 个数量级（$delta alpha approx 5.4 times 10^(-22)$，取 $alpha = 10^(-6)$、$Lambda = 10 "TeV"$、$lambda = 1 "m"$），比核钟可达性低 10 个数量级以上——核钟的态相关搜索因此是零背景证伪检验（脚本 17，图 7）。
...
正能定理（Schoen-Yau；Witten；2026 年扩展至任意端）断言：满足主能条件的孤立系统 ADM 质量非负。工程推论：普通物质（满足主能条件）的任意构型在无穷远处无法产生净排斥引力——"反引力源"在经典源通道内不存在。源通道的量子侧由量子不等式约束：负能量密度的幅度与持续时间成反比，宏观维持被排除（其不等式地位的争议不影响该排除结论，见配套文献）。
...
其中 $phi$ 为 KK 标量模式、$Lambda$ 为 EFT 截断。静态极限下 $partial^2 phi arrow.r m_"KK"^2 phi = phi/lambda^2$。态相关势与普适 Yukawa 势之比为

$ (delta alpha)/(alpha) = (c Q_"nuc" M_"Pl")/(sqrt(alpha) Lambda^2 lambda^2 m_"nuc"). $ <eq-sdratio>

三点注意。(i) $delta alpha$ 随 $sqrt(alpha)$ 增长——$alpha$ 越小态相关通道相对越强，但其绝对量在窗口内始终远低于可达性（见脚本 17 的表）。(ii) 可观测性要求截断降至约 43 MeV（窗口边缘 $alpha = 10^(-6)$；可达边缘 $alpha = 10^(-12)$ 时约 1 MeV）——即截断降到强子/QCD 尺度以下，局域 EFT 描述自身失效。(iii) 宏观 $lambda$ 下 MeV 截断的新物理早已被第五力实验排除。三点合起来：*态相关信号可观测，当且仅当局域 EFT 描述失效*。故核钟进阶阶段（微波调控核内部态）是一个量化了阈值的零背景证伪检验，其证伪对象覆盖 GR、标准 KK 与全部弱耦合局域 EFT；全部数字由 experiments/17 给出，图 7 给出可视化的零背景判据。
